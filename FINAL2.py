...
def ask_huggingface(prompt, model_url):
    """Бесплатные модели через Hugging Face"""
    try:
...
def process_ai(text):
    ...
    if current_ai_model != "myxa":
        ...
        # Hugging Face модели
        if current_ai_model == "dialogpt":
            return ask_huggingface(text, "https://api-inference.huggingface.co/models/microsoft/DialoGPT-medium")
        if current_ai_model == "flan-t5":
            return ask_huggingface(text, "https://api-inference.huggingface.co/models/google/flan-t5-small")
        if current_ai_model == "saiga":
            return ask_huggingface(text, "https://api-inference.huggingface.co/models/IlyaGusev/saiga_llama3_8b")
        
        # GigaChat
        if current_ai_model == "gigachat":
            return ask_gigachat(text)
...
def open_help():
    ...
    help_text = (
        ...
        "📚 ШКОЛА: таблица 5\n\n"
        "🧠 МОДЕЛИ: кнопка «🧠 Нейро» — выбор нейросети"
    )
...
def choose_neural_model():
    ...
    def set_model(model):
        ...
        add_message("MYXA AI", f"✅ Нейросеть: {names.get(model, model)}")
        win.destroy()
    ...
    models = [
        ...
        ("🌪️ Mistral (Ollama)", "mistral"),
        ("🧠 DeepSeek-R1 (Ollama)", "deepseek-r1:7b"),
        ("💬 DialoGPT (Бесплатно)", "dialogpt"),
        ("📚 Flan-T5 (Бесплатно)", "flan-t5"),
        ("🇷🇺 Saiga (Бесплатно)", "saiga"),
        ("🤖 GigaChat (Сбер)", "gigachat")
    ]
    
    for text, model in models:
        tk.Button(win, text=text, width=30, command=lambda m=model: set_model(m)).pack(pady=3)
    
    tk.Button(win, text="❌ Закрыть", command=win.destroy, width=20).pack(pady=15)
...
def toggle_deep_thinking():
    ...
    add_message("MYXA AI", f"🧠 Глубокое мышление: {status}")
    
    # Автоматически переключаем на DeepSeek при включении
    if deep_thinking:
        current_ai_model = "deepseek-r1:7b"
        add_message("MYXA AI", "🤖 Модель изменена на DeepSeek-R1")

def open_calculator():
    """Окно калькулятора"""
    calc_win = tk.Toplevel(root)
    calc_win.title("🧮 Калькулятор")
    calc_win.geometry("280x380")
    calc_win.resizable(False, False)
    calc_win.configure(bg="#2c2c2c")
    
    # Дисплей
    entry = tk.Entry(calc_win, font=("Arial", 20), justify="right", bg="#1e1e1e", fg="white", bd=5)
    ...
    for row in buttons:
        frame = tk.Frame(calc_win, bg="#2c2c2c")
        frame.pack(fill="x", padx=5, pady=2)
        for char in row:
            if char == 'C':
                btn = tk.Button(frame, text=char, width=5, height=2, command=clear, bg="#ff5252", fg="white")
            elif char == '⌫':
                btn = tk.Button(frame, text=char, width=5, height=2, command=backspace, bg="#ffa500", fg="white")
            elif char == '=':
                btn = tk.Button(frame, text=char, width=5, height=2, command=calculate, bg="#4CAF50", fg="white")
            elif char in ('+', '-', '×', '÷', '%'):
                btn = tk.Button(frame, text=char, width=5, height=2, command=lambda c=char: click(c), bg="#555", fg="white")
            elif char == '+/-':
                btn = tk.Button(frame, text=char, width=5, height=2, command=lambda: click('-'), bg="#555", fg="white")
            else:
                btn = tk.Button(frame, text=char, width=5, height=2, command=lambda c=char: click(c), bg="#333", fg="white")
            btn.pack(side="left", expand=True, fill="both", padx=1)
    
    tk.Button(calc_win, text="❌ Закрыть", command=calc_win.destroy, bg="#666", fg="white").pack(pady=5, fill="x", padx=10)
